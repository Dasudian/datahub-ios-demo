...
3. 如果想接收消息，那么就订阅某个topic

4. 通过publish函数发送消息到服务器
...
建议创建一个子线程单独调用。因为当消息发送过快而导致消息队列满了，此时SDK会休眠等待消息发送出去。
