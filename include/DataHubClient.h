...
/**
 * @deprecated 由于异步发送消息的个数有限制, 不建议使用该接口; 且后期版本会废弃
 * 该接口
 * @brief 异步发送消息
 *
 * @warning 异步操作不阻塞线程,但不能保证消息发送成功,适用于对时间敏感,对消息成功
 * 与否不敏感的应用
 ...
 * @brief 同步上传图片
 *
 * @deprecated MQTT协议不适合大消息, 不建议使用, 以后将取消该接口
 * @warning 注意：程序会阻塞
 * @param client 由函数datahub_create()成功返回的客户端实例<br>
 * 注意: 不能为空<br>
 * @param topic 消息对应的topic. 如果消息发送前有另一个客户端已经订阅该topic,则
 ...
 * <b>2</b>: 消息一定会到达,且只到达一次,当然,前提是返回ERROR_NONE<br>
 * 注意：只能为0,1,2三者中的一个,其他为非法参数<br>
 * @param timeout 函数阻塞的最大时间.
 * 注意: 这是函数阻塞的最大时间,不是消息的超时时间<br>
 * @return
 * ERROR_NONE 表示成功,消息一定发送出去.<br>
 * ERROR_TIMEOUT 表示阻塞等待时间的最大值已到,但是消息可能发送给服务器,也可能未
 * 发送.如果想确保消息一定发送出去,请根据消息大小和网络状况设置较大的阻塞等待时
 * 间.<br>
 * 其他错误码请查看开发文档<br>
 */
